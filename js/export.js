...
(function (exports) {
   ...
   */
  function exportToJSON(results, filename = `axioma_pvq_rr_results_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.json`) {
    if (!results) return;
  ...
  function exportToTSV(results, filename = `axioma_pvq_rr_results_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.tsv`) {
   ...
   * Tab 5: Token Usage
   */
  function exportToXLSX(results, filename = `axioma_pvq_rr_results_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.xlsx`) {
    if (!results || !results.psychometrics) return;
   ...
   * @param {string} [filename] - Output filename
   */
  function exportSessionToCSV(sessionResults, filename = `axioma_session_results_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.csv`) {
   ...
   * @param {string} [filename] - Output filename
   */
  function exportSessionToXLSX(sessionResults, filename = `axioma_session_results_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.xlsx`) {
    if (!sessionResults || !Array.isArray(sessionResults) || sessionResults.length === 0) return;
    if (typeof window.XLSX === 'undefined') {
      alert("XLSX library not loaded. Please ensure you have internet access or CDN loaded.");
      return;
