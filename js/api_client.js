      ...
      try {
        ...
        if (isRateLimit && attempt < maxRetries) {
          ...
          if (onStatusUpdate) {
            ...
            onStatusUpdate(`Rate limit (429) hit. Waiting ${sec}s before retry ${attempt + 1}/${maxRetries}...`);
            const intervalId = setInterval(() => {
              sec--;
              if (sec > 0) onStatusUpdate(`Rate limit (429) hit. Waiting ${sec}s before retry ${attempt + 1}/${maxRetries}...`);
            }, 1000);
            await delay(totalSeconds * 1000);
