...
(function () {
  ...
  let modalRefinedChartInstance = null;
  ...
  function renderCharts() {
      ...
      higherOrderChartInstance = new Chart(hoCanvas.getContext('2d'), {
        ...
        options: {
          ...
          onClick: (evt, activeElements) => {
            const hoKeys = Object.keys(psych.higherOrderValues);
            if (activeElements && activeElements.length > 0) {
              const clickedIndex = activeElements[0].index;
              state.selectedDimension = hoKeys[clickedIndex] || 'ALL';
            } else {
              state.selectedDimension = 'ALL';
            }
            state.showRefinedModal = true;
            renderApp();
          },
          scales: {
            r: {
    ...
    renderModalRefinedChart();
  }

  function renderModalRefinedChart() {
    if (!state.showRefinedModal || !state.results || !state.results.psychometrics) return;

    const modalCanvas = document.getElementById('modalRefinedChartCanvas');
    if (!modalCanvas || !window.Chart) return;

    if (modalRefinedChartInstance) modalRefinedChartInstance.destroy();

    const psych = state.results.psychometrics;
    const refinedList = Object.values(psych.refinedValues);

    const labels = refinedList.map(v => `${v.code} (${state.lang === 'pl' ? v.namePl : v.nameEn})`);
    const rawScores = refinedList.map(v => v.rawMean ?? 0);

    const higherOrderColors = {
      Openness: { bg: 'rgba(56, 189, 248, 0.75)', activeBg: 'rgba(56, 189, 248, 0.95)', border: '#0ea5e9' },
      Enhancement: { bg: 'rgba(244, 63, 94, 0.75)', activeBg: 'rgba(244, 63, 94, 0.95)', border: '#f43f5e' },
      Conservation: { bg: 'rgba(245, 158, 11, 0.75)', activeBg: 'rgba(245, 158, 11, 0.95)', border: '#f59e0b' },
      Transcendence: { bg: 'rgba(168, 85, 247, 0.75)', activeBg: 'rgba(168, 85, 247, 0.95)', border: '#a855f7' }
    };

    const backgroundColors = refinedList.map(v => {
      const hoKey = v.higherOrder;
      const isSelected = state.selectedDimension === 'ALL' || state.selectedDimension === hoKey;
      const theme = higherOrderColors[hoKey] || { bg: 'rgba(148, 163, 184, 0.5)', activeBg: 'rgba(148, 163, 184, 0.9)', border: '#94a3b8' };
      return isSelected ? theme.activeBg : 'rgba(51, 65, 85, 0.3)';
    });

    const borderColors = refinedList.map(v => {
      const hoKey = v.higherOrder;
      const isSelected = state.selectedDimension === 'ALL' || state.selectedDimension === hoKey;
      const theme = higherOrderColors[hoKey] || { bg: 'rgba(148, 163, 184, 0.5)', activeBg: 'rgba(148, 163, 184, 0.9)', border: '#94a3b8' };
      return isSelected ? theme.border : '#475569';
    });

    modalRefinedChartInstance = new Chart(modalCanvas.getContext('2d'), {
      type: 'polarArea',
      data: {
        labels: labels,
        datasets: [{
          label: state.lang === 'pl' ? 'Wynik surowy (1-6)' : 'Raw Score (1-6)',
          data: rawScores,
      ...
      options: {
        ...
        scales: {
          r: {
            ...
            pointLabels: { display: true, centerPointLabels: true, color: '#f8fafc', font: { size: 10, weight: 'bold' } }
          }
        },
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (context) => {
                const item = refinedList[context.dataIndex];
                const raw = item.rawMean !== null ? item.rawMean : 'N/A';
                const cent = item.centeredMean !== null ? (item.centeredMean > 0 ? `+${item.centeredMean}` : item.centeredMean) : 'N/A';
                return [
                  ` ${state.lang === 'pl' ? 'Średnia surowa' : 'Raw Mean'}: ${raw}`,
                  ` ${state.lang === 'pl' ? 'Wycentrowany' : 'Centered'}: ${cent}`,
                  ` ${state.lang === 'pl' ? 'Wymiar nadrzędny' : 'Higher-Order'}: ${item.higherOrder}`
                ];
              }
            }
          }
  ...
  function refinedCircleModal() {
    if (!state.showRefinedModal || !state.results || !state.results.psychometrics) return null;

    const psych = state.results.psychometrics;
    const hoValues = psych.higherOrderValues;
    const refinedValues = psych.refinedValues;

    const filteredRefined = Object.values(refinedValues).filter(rv => {
      return state.selectedDimension === 'ALL' || rv.higherOrder === state.selectedDimension;
    });

    return el('div', { className: 'fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto' },
      el('div', { className: 'glass-panel bg-slate-900 border border-slate-800 rounded-2xl max-w-5xl w-full p-6 space-y-6 shadow-2xl my-8 max-h-[90vh] flex flex-col' },
        // Header
        el('div', { className: 'flex items-center justify-between border-b border-slate-800 pb-4' },
          el('div', { className: 'flex items-center space-x-3' },
            el('div', { className: 'w-10 h-10 rounded-xl bg-sky-500/20 border border-sky-500/40 flex items-center justify-center text-sky-400 font-bold text-lg' },
              el('i', { className: 'fa-solid fa-circle-notch' })
            ),
            el('div', {},
              el('h3', { className: 'text-lg font-bold text-slate-100' },
                state.lang === 'pl' ? 'Koło 19 Szczegółowych Wartości Podstawowych (PVQ-RR)' : 'Refined Basic Values Circle (19 Values)'
              ),
              el('p', { className: 'text-xs text-slate-400' },
                state.lang === 'pl'
                  ? 'Powiększony wykres struktur kołowych Schwartza na podstawie oceny psychometrycznej'
                  : 'Enlarged Schwartz Refined Basic Values circular structure from assessment'
              )
            )
          ),
          el('button', {
            onClick: () => { state.showRefinedModal = false; renderApp(); },
            className: 'text-slate-400 hover:text-slate-200 text-xl px-2 py-1'
          }, el('i', { className: 'fa-solid fa-xmark' }))
        ),

        // Content Area (Filter Pills + Diagram & Table)
        el('div', { className: 'overflow-y-auto space-y-6 pr-1 flex-1' },
          // Filter Buttons
          el('div', { className: 'flex flex-wrap items-center gap-2 border-b border-slate-800/80 pb-3' },
            el('span', { className: 'text-xs font-semibold text-slate-400 mr-2' },
              state.lang === 'pl' ? 'Filtruj wg wymiaru:' : 'Filter Dimension:'
            ),
            el('button', {
              onClick: () => { state.selectedDimension = 'ALL'; renderApp(); },
              className: `px-3 py-1 text-xs font-medium rounded-lg border transition ${state.selectedDimension === 'ALL' ? 'bg-sky-500 text-white border-sky-400 shadow-md shadow-sky-500/20' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-slate-200'}`
            }, state.lang === 'pl' ? 'Wszystkie 19 wartości' : 'All 19 Values'),
            Object.values(hoValues).map(ho =>
              el('button', {
                key: ho.code,
                onClick: () => { state.selectedDimension = ho.code; renderApp(); },
                className: `px-3 py-1 text-xs font-medium rounded-lg border transition ${state.selectedDimension === ho.code ? 'bg-sky-500 text-white border-sky-400 shadow-md shadow-sky-500/20' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-slate-200'}`
              }, state.lang === 'pl' ? ho.namePl : ho.nameEn)
            )
          ),

          // Enlarged Polar Chart Container
          el('div', { className: 'bg-slate-950/60 p-5 rounded-2xl border border-slate-800 space-y-2' },
            el('div', { className: 'flex items-center justify-between text-xs text-slate-400' },
              el('span', { className: 'font-semibold text-sky-400' },
                state.selectedDimension === 'ALL'
                  ? (state.lang === 'pl' ? 'Koło Wartości Podstawowych (Wszystkie 19 Wartości)' : 'Refined Basic Values Circle (All 19 Values)')
                  : `${state.lang === 'pl' ? 'Wybrany wymiar' : 'Selected Dimension'}: ${state.lang === 'pl' ? hoValues[state.selectedDimension]?.namePl : hoValues[state.selectedDimension]?.nameEn}`
              ),
              el('span', { className: 'text-[11px] code-font text-slate-500' }, 'Scale: 1 (Not at all) - 6 (Very much)')
            ),
            el('div', { className: 'h-80 relative flex items-center justify-center' },
              el('canvas', { id: 'modalRefinedChartCanvas' })
            )
          ),

          // Detailed Table of Filtered Refined Basic Values
          el('div', { className: 'space-y-3' },
            el('h4', { className: 'text-sm font-semibold text-slate-200 flex items-center gap-2' },
              el('i', { className: 'fa-solid fa-list text-sky-400' }),
              state.lang === 'pl' ? 'Szczegółowe wyniki i pozycje PVQ-RR' : 'Detailed Value Scores & Constituent Items'
            ),
            el('div', { className: 'overflow-x-auto bg-slate-950/40 rounded-xl border border-slate-800' },
              el('table', { className: 'w-full text-left text-xs border-collapse' },
                el('thead', {},
                  el('tr', { className: 'border-b border-slate-800 text-slate-400 bg-slate-900/60' },
                    el('th', { className: 'py-2.5 px-3 w-16' }, 'Code'),
                    el('th', { className: 'py-2.5 px-3' }, state.lang === 'pl' ? 'Nazwa wartości' : 'Refined Value Name'),
                    el('th', { className: 'py-2.5 px-3 w-32' }, state.lang === 'pl' ? 'Wymiar nadrzędny' : 'Higher-Order Group'),
                    el('th', { className: 'py-2.5 px-3 w-28' }, state.lang === 'pl' ? 'Numery pytań' : 'PVQ Items'),
                    el('th', { className: 'py-2.5 px-3 w-24' }, state.lang === 'pl' ? 'Średnia surowa' : 'Raw Mean'),
                    el('th', { className: 'py-2.5 px-3 w-28' }, state.lang === 'pl' ? 'Wynik wycentrowany' : 'Centered Score')
                  )
                ),
                el('tbody', { className: 'divide-y divide-slate-800/50' },
                  filteredRefined.map(rv =>
                    el('tr', { key: rv.code, className: 'hover:bg-slate-800/30' },
                      el('td', { className: 'py-2 px-3 font-bold text-sky-400 code-font' }, rv.code),
                      el('td', { className: 'py-2 px-3 text-slate-200 font-medium' }, state.lang === 'pl' ? rv.namePl : rv.nameEn),
                      el('td', { className: 'py-2 px-3 text-slate-400' }, hoValues[rv.higherOrder] ? (state.lang === 'pl' ? hoValues[rv.higherOrder].namePl : hoValues[rv.higherOrder].nameEn) : rv.higherOrder),
        ...
        // Footer
        el('div', { className: 'pt-2 border-t border-slate-800 flex justify-between items-center' },
          el('span', { className: 'text-xs text-slate-500' },
            state.lang === 'pl' ? 'Skala Schwarza PVQ-RR (1-6) z centrowaniem MRAT' : 'Schwartz PVQ-RR Scale (1-6) with MRAT Grand-Mean Centering'
          ),
          el('button', {
            onClick: () => { state.showRefinedModal = false; renderApp(); },
            className: 'bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-5 py-2 rounded-xl transition'
          }, state.lang === 'pl' ? 'Zamknij' : 'Close')
        )
      )
    );
  }
  ...
  function ResultsTab() {
    ...
    return el('div', { className: 'space-y-8' },
      ...
      el('div', { className: 'grid grid-cols-1 lg:grid-cols-2 gap-6' },
        el('div', {
          className: 'bg-slate-950/50 p-4 rounded-xl border border-slate-800 hover:border-sky-500/50 cursor-pointer transition group relative',
          onClick: () => {
            if (!state.showRefinedModal) {
              state.showRefinedModal = true;
              renderApp();
            }
          }
          ...
          el('div', { className: 'flex items-center justify-between mb-3' },
            el('h3', { className: 'text-xs font-bold text-slate-300 uppercase tracking-wider group-hover:text-sky-400 transition' }, '4 Higher-Order Value Dimensions'),
            el('span', { className: 'text-[10px] bg-sky-500/20 text-sky-300 px-2 py-0.5 rounded-full font-medium flex items-center gap-1 group-hover:bg-sky-500 group-hover:text-white transition' },
              el('i', { className: 'fa-solid fa-expand' }),
              state.lang === 'pl' ? 'Powiększ koło wartości' : 'Click to enlarge Refined Circle'
            )
          ),
  ...
  // Expose mock injector for testing/verification
  window.setMockResults = function(mockResult, openModal = true) {
    state.results = mockResult;
    if (openModal) state.showRefinedModal = true;
    renderApp();
  };
