      ...
      el('div', { className: 'max-w-7xl mx-auto px-4 py-4 flex flex-wrap items-center justify-between gap-4' },
        ...
        el('div', { className: 'flex items-center space-x-3' },
          ...
          state.sessionResults.length > 0 ? el('div', { className: 'flex space-x-2' },
            el('button', {
              onClick: () => window.DataExporter.exportSessionToCSV(state.sessionResults, `Axioma_Session_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.csv`),
            ...
            el('button', {
              onClick: () => window.DataExporter.exportSessionToXLSX(state.sessionResults, `Axioma_Session_${new Date().toISOString().replace('T', '_').replace(/:/g, '-').slice(0, 19)}.xlsx`),
